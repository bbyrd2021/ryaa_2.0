...
def route_calendar_request(user_input: str) -> CalendarRequestType:
    ...
    if result is None:
        raise ValueError("Failed to parse calendar request type")
    logger.info(
...
def handle_new_event(description: str) -> CalendarResponse:
    ...
    if details is None:
        raise ValueError("Failed to parse new event details")
...
def handle_modify_event(description: str) -> CalendarResponse:
    ...
    details = completion.choices[0].message.parsed
    if details is None:
        raise ValueError("Failed to parse modify event details")
