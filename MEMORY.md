...
- ✅ **REAL calendar backend** (2026-05-31) — `AppleCalendar` (osascript) creates real macOS Calendar.app events; verified live. Brandon's NC A&T cal (`blbyrd@aggies.ncat.edu`) is synced into Calendar.app → **real school events with ZERO Azure** (the long pole is dead). Committed `8c4c4b8`.
- ✅ **Frontend started** (2026-05-31) — `frontend/` Vite + React 19 + TypeScript chat UI. **Decision D6:** FastAPI backend + TS web frontend. Design system is pure CSS (grainy Frutiger-Aero gradient + soft-UI pebble bubbles + glass edge-lighting + Quicksand/Hanken type). ⚠️ Tried `liquid-glass-react` — abandoned (React-19 incompat + designed for single cards not scrolling lists; lesson: flashy deps that fight the framework lose to solid CSS you control). ⚠️ **The frontend is still MOCK** — `handleSend` fakes replies; NOT wired to the Python brain yet.

**Next up (START HERE):** (1) **`ryaa/api.py` FastAPI layer** exposing `Scheduler.propose`/`create` as `/propose` + `/confirm` (+ CORS); (2) **wire the frontend** `handleSend` → fetch `/propose` → render proposal bubble + Confirm/Cancel → `/confirm` → real AppleCalendar event (browser = the confirmer); (3) regression test for "created" message; then push to GitHub + LinkedIn, then persistence+Todos → Email → Study Coach → Morning Brief → Dream Mode. Detail: `docs/session-notes/2026-05-31.md`.
